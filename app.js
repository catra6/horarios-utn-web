...
function renderGridStructure() {
    ...
    // Cabecera superior izquierda (Vacía según especificación)
    const timeHeader = document.createElement('div');
    timeHeader.className = 'grid-header grid-header-time';
    timeHeader.innerText = '';
    gridContainer.appendChild(timeHeader);

    // Cabeceras de días (Nombre completo y abreviado 1 letra L M M J V S D)
    const dayShortNames = { 1: 'L', 2: 'M', 3: 'M', 4: 'J', 5: 'V', 6: 'S', 7: 'D' };
    activeDays.forEach((dayObj, idx) => {
        const header = document.createElement('div');
        header.className = 'grid-header';
        header.style.gridRow = '1';
        header.style.gridColumn = `${idx + 2}`;
        const shortName = dayShortNames[dayObj.id] || dayObj.name.charAt(0);
        header.innerHTML = `<span class="day-full">${dayObj.name}</span><span class="day-short">${shortName}</span>`;
...
function toTitleCase(str) {
    ...
    const minorWords = new Set(['y', 'de', 'del', 'la', 'los', 'las', 'e', 'o', 'a', 'en']);

    return str.toLowerCase().split(' ').map((word, idx) => {
        if (!word) return '';
        const cleanWord = word.replace(/[^a-záéíóúñü]/g, '');
        if (romanNumerals.has(cleanWord)) {
...
function abbreviateSubjectName(name) {
    ...
    const abbrevMap = {
        ...
        'electrónica': 'Electr.',
        'electronica': 'Electr.',
        'electrotécnica': 'Electr.',
        'electrotecnica': 'Electr.'
    };
...
function renderSubjectsList() {
    ...
    state.subjects.forEach(subject => {
        ...
        const nameText = document.createTextNode(toTitleCase(subject.name) + ' ');
...
function updateTimetable() {
    ...
    activeSlots.forEach(slot => {
        ...
        if (isConflicting) {
            ...
            const fullSubjectName = toTitleCase(slot.subjectName);
            const abbrevSubjectName = abbreviateSubjectName(slot.subjectName);
            const fullTooltip = `${fullSubjectName}${slot.commissionName ? ' - Com. ' + slot.commissionName : ''} (${slot.startTime} - ${slot.endTime})${slot.classroom ? ' [' + slot.classroom + ']' : ''}${slot.teacher ? ' - Prof. ' + slot.teacher : ''}`;

            eventEl.title = fullTooltip;
            eventEl.innerHTML = `
                <div class="event-subject" title="${fullTooltip}">
                    <span class="subject-full-name">${fullSubjectName}</span>
                    <span class="subject-short-name">${abbrevSubjectName}</span>
                </div>
                ${commTitle}
                ${classroomHTML}
                ${teacherHTML}
...
function initTableSize() {
    ...
    if (isNaN(val) || val > 1.18 || val < 0.15) {
        val = isMobile ? 0.25 : 0.81;
    }
...
function initExportChoiceFeature() {
    const openBtn = document.getElementById('open-export-modal-btn');
    const modal = document.getElementById('export-choice-modal');
    const closeBtn = document.getElementById('export-choice-close-btn');
    const choicePdfBtn = document.getElementById('choice-pdf-btn');
    const choiceGCalBtn = document.getElementById('choice-gcal-btn');
    ...
    if (openBtn) openBtn.addEventListener('click', openChoiceModal);
    if (closeBtn) closeBtn.addEventListener('click', closeChoiceModal);
    ...
    if (choicePdfBtn) {
        choicePdfBtn.addEventListener('click', () => {
            closeChoiceModal();
            openPdfModal();
        });
    }
    ...
    if (choiceGCalBtn) {
        choiceGCalBtn.addEventListener('click', () => {
            closeChoiceModal();
            openGCalModal();
        });
    }
...
function initWelcomeModal() {
    ...
    const dontShowCheckbox = document.getElementById('welcome-dont-show-again');

    const dontShow = localStorage.getItem('horauni_dont_show_welcome');
    ...
    function closeModal() {
        if (dontShowCheckbox && dontShowCheckbox.checked) {
            localStorage.setItem('horauni_dont_show_welcome', 'true');
        }
        modal.classList.add('hidden');
    }
    ...
    if (dontShowCheckbox) {
        dontShowCheckbox.addEventListener('change', () => {
            if (dontShowCheckbox.checked) {
                ...
                localStorage.removeItem('horauni_dont_show_welcome');
            }
        });
    }
